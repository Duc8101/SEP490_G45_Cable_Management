     ...
     CableId = Guid.NewGuid(), StartPoint = 0, EndPoint = 100
        ...
        public async Task SuggestionCable_WhenWarehouseChosen_ReturnsCorrectResponse()
        {
            ...
            var suggestionCableDTOSample =
                new SuggestionCableDTO
                {
                    Length = 100,  // Replace with a valid length
                    WarehouseIds = new List<int> { 1, 2 },
                    CableCategoryIds = new List<int> { 1, 2 }
                };

            var resultSample = new List<CableListDTO> {
    new CableListDTO { CableId = Guid.NewGuid(), WarehouseId = 1,
                       WarehouseName = "Sample Warehouse", SupplierId = 1,
                       SupplierName = "Sample Supplier", StartPoint = 0, EndPoint = 100,
                       Length = 100, YearOfManufacture = 23, Code = "Sample Code",
                       Status = "Sample Status", IsExportedToUse = true, IsInRequest = false,
                       CableCategoryId = 1, CableCategoryName = "Sample Category" },
    new CableListDTO { CableId = Guid.NewGuid(), WarehouseId = 2,
                       WarehouseName = "Sample Warehouse 2", SupplierId = 2,
                       SupplierName = "Sample Supplier 2", StartPoint = 0, EndPoint = 200,
                       Length = 200, YearOfManufacture = 22, Code = "Sample Code 2",
                       Status = "Sample Status 2", IsExportedToUse = false, IsInRequest = true,
                       CableCategoryId = 2, CableCategoryName = "Sample Category 2" }
   };

            TestHelper.SimulateUserWithRoleAndId(controller, RoleConst.STRING_ROLE_WAREHOUSE_KEEPER);

            requestService.Setup(x => x.SuggestionCable(suggestionCableDTOSample))
                .ReturnsAsync(new ResponseDTO<List<CableListDTO>?>(resultSample, string.Empty));

            // Act
            var result = await controller.SuggestionCable(suggestionCableDTOSample);

            requestService.Verify(x => x.SuggestionCable(suggestionCableDTOSample));
